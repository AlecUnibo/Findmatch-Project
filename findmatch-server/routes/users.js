...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Errore del server durante la ricerca utenti.' })
  }
})
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Errore del server nel recupero dell\'utente.' });
  }
})
...
router.put('/change-password', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Errore del server durante l\'aggiornamento della password.' })
  }
})
